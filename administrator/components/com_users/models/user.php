...
class UsersModelUser extends JModelAdmin
{
	...
	public static function setPrevisiones($data){
		$lista_prevision = $data['lista_prevision'];
		foreach($lista_prevision as $key => $prevision) {
			if($prevision['activo'] == 1 && !$prevision['activo_old']){
				self::agregar_registro_prevision($data['id'],$prevision['id']);
			}
			if(!$prevision['activo'] && $prevision['activo_old'] == true){
				self::eliminar_registro_prevision($data['id'], $prevision['id']);
			}
		}
	}
