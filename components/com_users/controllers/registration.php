...
class UsersControllerRegistration extends UsersController
{
	...
	public function register(){
		...
		$rut = $requestData['rut'];
		if($this->valida_rut($rut) == false){
			$app->enqueueMessage("RUT Inválido", 'error');
		...
		if(strlen($telefono) != 9 || $telefono[0] != "9"){
			$app->enqueueMessage("El teléfono debe tener 9 carácteres, y el primer dígito debe ser un número nueve.", 'error');
			$app->setUserState('com_users.registration.data', $requestData);
			$this->setRedirect(JRoute::_('index.php?option=com_users&view=registration' . $string_tipo, false));
			return false;	
		}
	...
	public function enviarCorreoAdministrador($usuario){
